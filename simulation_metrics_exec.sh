...
echo "$(($(date -d "$t2" +%s%9N) - $(date -d "$t1" +%s%9N)))" >> results/execution_time
...
mkdir -p results
sudo docker exec -it influxdb sh -c 'influx query -f query -r' > results/metrics_output

echo "*************************************************"
echo "File metrics_output created"
echo "*************************************************"
head results/metrics_output
echo "*************************************************"
