...
echo "$(($(date -d "$t2" +%s%9N) - $(date -d "$t1" +%s%9N)))" >> simulations/results/execution_time
...
sudo docker exec -it influxdb sh -c 'influx query -f query -r' > simulations/results/metrics_output

echo "*************************************************"
echo "File metrics_output created"
echo "*************************************************"
head simulations/results/metrics_output
echo "*************************************************"
