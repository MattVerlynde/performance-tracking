...
if __name__ == "__main__":
    ...
    parser.add_argument("--storage_path", type=str, default="simulations/results_qanat/conso-clustering-blob")
    ...
    if not args.file:
        ...
        for group_i, id in enumerate(args.id):
            if not args.grouped[group_i]:
                ...
                random_seed = int(paramYaml['parameters']['--random_seed'])
                data_seed = float(paramYaml['parameters']['--data_seed'])
                model = paramYaml['parameters']['--model']
                repeat = int(paramYaml['parameters']['--repeat'])
                ...
                output_df_i["N clusters"] = n_clusters*np.ones(len(output_df_i))
                output_df_i["Repeat"] = repeat*np.ones(len(output_df_i))
                output_df_i["Model"] = [model for x in range(len(output_df_i))]
                output_df_i["Random seed"] = random_seed*np.ones(len(output_df_i))
                output_df_i["Data seed"] = data_seed*np.ones(len(output_df_i))
